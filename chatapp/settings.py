...
# Render provides RENDER_EXTERNAL_HOSTNAME; accept all hosts if not specified
_allowed_env = os.environ.get('ALLOWED_HOSTS', '')
_render_host = os.environ.get('RENDER_EXTERNAL_HOSTNAME', '')
_hosts = [h.strip() for h in _allowed_env.split(',') if h.strip()]
...
# ── Channel Layers ──────────────────────────────────────────────────────────
# Use Redis if REDIS_URL is set, otherwise fall back to InMemory (single-server)
...
if _REDIS_URL:
    ...
    # InMemory channel layer — works without Redis but does NOT support
    # multi-process deployments. Fine for single-dyno / local dev.
    CHANNEL_LAYERS = {
        'default': {
